...
TableReadResult ReadHeadingTable(hid_t fd, const std::string_view name) {
    ...
    if (dset == H5I_INVALID_HID) {
        std::cout << "Unable to find dataset `" << name << "`" << std::endl;
        return res;
    }

    hid_t space = H5Dget_space(dset);
    if (space == H5I_INVALID_HID) {
        std::cout << "Unable to gen data space" << std::endl;
        H5Dclose(dset);
        return res;
    }
    ...
    if (rank != static_cast<hsize_t>(2)) {
        std::cout << "Table expected, rank = " << rank << " != " << 2
                  << std::endl;
        H5Sclose(space);
        H5Dclose(dset);
        return res;
...
space::I16 Hdf5Reader::ReadI16(std::string_view dset_name) {
    space::I16 res;
    std::cout << "Try to read " << dset_name << " [I16]..." << std::flush;
